...
How every feature gets verified before it ships. Layered, fast to thorough. **The running shell is
...
and Wayland test tooling (June 2026); see the tool table at the bottom.
...
### 1. Static gate: `just lint` (authoritative)
`just lint` feeds qmllint the session `$QML_IMPORT_PATH` (`-I …`) so QtQuick/Quickshell types resolve
(false positives drop ~16 to 2 per file) and filters the categories that stay structurally
unresolvable on NixOS (the `qs.*` config imports plus project singletons). What survives is real: it
catches a typo'd property (`heigth`) that the old noisy lint buried. Also grep the injection
antipattern:
```sh
...
There is no parse-only flag and QML errors do NOT exit the process, so the gate is this: load the
whole shell in an isolated instance and assert it reaches "Configuration Loaded" with no errors in
your files.
```sh
...
```
Ignore the polkit / UPower / AiChat / KeyringStorage warnings; they are duplicate-instance artifacts.

### 3. Service logic: deterministic parse tests (the bug-catcher)
For anything parsing `nmcli`/`pactl`/`playerctl`/leases output, **test the parse against canned
output**, not live hardware. Two ways:
- **Fake-bin PATH shim**: drop a fake `nmcli` printing a known line into a temp dir on `$PATH`, run
  the service, assert. (Quickshell `Process` calls `execve` and inherits `$PATH`, so this intercepts
  even `bash -c "nmcli …"`.)
...
`"deactivated"`; `includes("connected")` matching `"disconnected"`). When a feature depends on a
tool's exact output, **run the tool and look**. Don't assume (the `:activated` severity was over-rated
because an agent assumed the nmcli output).

### 4. IPC liveness: `qs ipc`
```sh
PID=$(pgrep -f 'quickshell -c ii')
qs ipc --pid $PID call <target> status   # service alive + sane state
qs ipc --pid $PID show                   # list all IpcHandler targets
```
Quickshell 0.3.0 (nixpkgs) runs through a wrapper, so bare `qs ipc` can't find the instance; pass
`--pid` (or `qs list --all` to discover it). Every service should expose a `status()` IpcHandler
returning its key state.
...
Run the multi-agent review on the diff (Sonnet, cost-controlled): the `qa-session-review` workflow,
finders per dimension (bugs/vulns/races/redundancy), each finding adversarially verified. It found the
real hotspot bugs this layer is meant to catch.

### 7. Security
- Injection: the grep in step 1. **Always pass tainted values via the Process `environment` map**,
  never interpolate into `bash -c`.
...
- Any externally-controlled string in a `StyledText` needs `textFormat: Text.PlainText` (StyledText
  defaults to AutoText = HTML; a DHCP hostname could inject markup).
...
reference recipe. Involved; current practice is the local loop above, and this is the upgrade path.
